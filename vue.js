...
(function (global, factory) {
  ...
  */
  ...
  // 这些助手在JS引擎中生成更好的VM代码，这是由于它们的明确性和函数内联。

  // 判断 是 undefined 或者 null
  function isUndef(v) {
    return v === undefined || v === null
  }

  // 判断 不是 undefined 和 null
  function isDef(v) {
    return v !== undefined && v !== null
  }
  ...
  function toRawType(value) {
    //类型判断 返会Array ，Function，String,Object,RegExp 等
   ...
   */
  // 将 null array object JSON 和 基础类型 转换为 字符串
   ...
   * is in that map.
   * 制作一个map并返回一个函数，用于检查键是否在该map中。
   * str 传入字符串 "num1,num2,num3" 转换为 [num1:true, num2:true, num3: true]
   */
  function makeMap(
    str,
    expectsLowerCase
    ...
    var list = str.split(','); // 字符串按逗号 分割成数组
    for (var i = 0; i < list.length; i++) { // 组装数组
      map[list[i]] = true;
    }
    return expectsLowerCase
      ? function (val) { return map[val.toLowerCase()]; }
      : function (val) { return map[val]; }
    // 返回 函数,持久化map,用于检查 str的num1 是否在 map 里面
  }

  /**
   * Check if a tag is a built-in tag.
   * 检查标记是否为内置标记。 检查slot 和 component
   */
